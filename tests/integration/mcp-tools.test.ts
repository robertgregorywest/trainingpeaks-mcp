  ...
  () => {
    ...
    describe("get_workout", () => {
      it("returns JSON with workoutId matching input", async () => {
        ...
        const listRaw = await getWorkouts(client, {
    ...
    describe("search_workouts", () => {
      it("returns matching workouts for known title", async () => {
        ...
        const list = JSON.parse(listRaw);
        const titled = list.find(
          (w: { title?: string }) => w.title && w.title.length > 3,
        );
        if (!titled) return;
    ...
    describe("get_best_power", () => {
      it.skipIf(!bikeWorkoutId)(
        "computes best power at multiple durations",
        async () => {
